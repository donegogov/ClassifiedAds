...
namespace FreeAds.API.Controllers
{
    ...
    public class ClassifiedAdsController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetClassifiedAdDetail(Guid id)
        {
            ...
            //classifiedAdsToReturn.UserPublishedUserName = classifiedAds.AppUser.UserName;
        ...
        {
            var user = await _userManager.Users.SingleOrDefaultAsync(x => x.Id.ToString().ToLower() == id.ToString().ToLower() && x.Id.ToString().ToLower() == User.GetUserId().ToString().ToLower());
            ...
            var classifiedAdsForUserUpdateFromRepo = await _repo.GetClassifiedAdDetaiForUser(classifiedAdsForUserUpdate.Id);
